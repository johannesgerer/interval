...
data Extended a = NegativeInfinity | PositiveInfinity | Finite !a
  deriving (Show, Eq, Functor, Generic, Hashable)

makeLensesWithSuffix ''Interval
makePrisms ''Extended
...
-- | [] -> unbounded
intersectionMany :: (Bounded a, Foldable f, Ord a) => f (Interval a) -> Maybe (Interval a)
intersectionMany = intersectionSome . (unbounded :|) . toList
{-# INLINABLE intersectionMany #-}

intersectionSome :: (Ord a) => NonEmpty (Interval a) -> Maybe (Interval a)
intersectionSome = foldl1' (\a b -> join $ liftA2 intersection a b) . fmap Just
...
-- | represented as a mapping from upper bound of the step to Just Value, or Nothing if not defined
--
-- examples:
-- λ> (toStepFunction [(hull @Int 2 3, 'a'), (hull @Int 4 maxBound, 'b')])
-- Right (fromList [(1,Nothing),(3,Just 'a'),(9223372036854775807,Just 'b')])
-- 
-- λ> (toStepFunction [(hull @Int 2 3, 'a'),(fromLowerBound 5, 'b')])
-- Right (fromList [(1,Nothing),(3,Just 'a'),(4,Nothing),(9223372036854775807,Just 'b')])
-- 
newtype StepFunction x y = UnsafeStepFunction { fromUnsafeStepFunction :: M.Map x (Maybe y) }
  deriving (Eq, Show) via M.Map x (Maybe y) deriving Functor
...
-- * JSON instances for StepFunctions

-- | see tests for examples
parseJSONStepFunction :: (FromJSON x, Ord x', Bounded x', Enum x', Show x', FromJSON y)
  => Maybe Key -> (x -> x') -> Value -> Parser (StepFunction x' y)
parseJSONStepFunction keyM modX =
  chain (either (fail . toS) pure . toStepFunction' . fmap fromPlateau) . \case
      Object o  -> pure <$> parsePlateau o
      Array arr -> toList <$> mapM (withObject "Plateau" parsePlateau) arr
      v         -> if isNothing keyM then
        typeMismatch "Object or Arrayu" v
        else pure . Plateau . (unbounded,) <$> parseJSON v
  where parsePlateau o = do
          from      <- maybe NegativeInfinity (Finite . modX) <$> (o .:? "from")
          to        <- maybe PositiveInfinity (Finite . modX) <$> (o .:? "to")
          let err = fail $ "empty step function plateau from " <> show from <> " to " <> show to
          fmap2 Plateau (,) <$> maybe err pure (fromBounds from to) <*> maybe (parseJSON $ Object o) (o .:) keyM

data Plateau x y = Plateau { fromPlateau :: (InfiniteInterval x, y) }
  deriving  (Show)
